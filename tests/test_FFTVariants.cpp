...
TEST(kernel_facts_are_found) {
  ...
  CHECK(carryTypedTypes() == (std::set<int>{FFT64, FFT32, FFT31, FFT61}));
...
TEST(carry_folds) {
  // Hybrids have fixed carry types.
  CHECK_EQ(canonicalSpec("4:1K:8:256:202:1"), std::string{"4:1K:8:256:202"});
  CHECK_EQ(canonicalSpec("50:256:2:256:202:0"), std::string{"50:256:2:256:202"});
