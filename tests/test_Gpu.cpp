...
TEST(args_copy_is_independent) {
  ...
  original.perFftConfig.push_back(tune::parseUseLine("! 512:15:512 PAD=256"));
  original.verbose = 1;

  Args copy = original;
  copy.flags["L2_STRIPING"] = "0";
  copy.flags["WMUL"] = "1";
  copy.perFftConfig.front().uses.clear();
  copy.verbose = 2;

  CHECK_EQ(original.value("L2_STRIPING", -1), 8);
  CHECK_EQ(original.value("WMUL", -1), -1);
  CHECK_EQ(original.perFftConfig.front().uses.size(), size_t{1});
  CHECK_EQ(original.verbose, 1);

  CHECK_EQ(copy.value("L2_STRIPING", -1), 0);
  CHECK_EQ(copy.value("WMUL", -1), 1);
  CHECK(copy.perFftConfig.front().uses.empty());
}
