...
public:
  ...
  Gpu(u32 E, u32 W, u32 BIG_H, u32 SMALL_H, int nW, int nH, cl_program program,
      cl_device_id device, cl_context context, bool timeKernels,
      bool useLongCarry);

  ~Gpu();

  void writeState(const std::vector<u32> &check, const std::vector<u32> &base,
                  const std::vector<u32> &gcdAcc, u32 blockSize);
  ...
  void dataLoopMul(const std::vector<bool> &muls) {
    modSqLoopMul(bufData, muls);
  }
  void dataLoopAcc(const std::vector<bool> &accs) {
