...
class KernelTest(unittest.TestCase):
    ...
    def planted(self, *mutations: Mutation) -> csi.Report:
        """Check shufl.cl with each mutation applied in turn."""
        text = self.sources.shufl

        for mutation in mutations:
            text = self.mutate(text, mutation)

        return csi.run_checks(replace(self.sources, shufl=text))
    ...
    def test_real_kernel_passes(self) -> None:
        ...
        self.assertEqual((report.special_checked, report.plain_checked, report.cross_checked), (72, 144, 36))
    ...
    def test_unread_arm_header_is_an_error(self) -> None:
        # Equivalent for the values SHUFL_BYTES takes, but not a form the checker reads.
        self.assertUnreadable(Mutation("if (SHUFL_BYTES == 16) {", "if (SHUFL_BYTES > 8) {", copies=2))

    def test_arm_chain_must_cover_every_shufl_bytes(self) -> None:
        self.assertUnreadable(Mutation("else if (SHUFL_BYTES == 4) {", "else if (SHUFL_BYTES == 2) {", copies=4))

    def test_changed_lds_base_pointer_is_an_error(self) -> None:
        self.assertUnreadable(Mutation("local T2_GF61* lds = lds2;", "local T2_GF61* lds = lds2 + 100000;", copies=2))
    ...
    def test_missing_barrier(self) -> None:
        ...
        report = self.planted(Mutation(anchor, anchor.replace("bar(WG);", "")))
        self.assertCaught(report, "not separated by bar(WG)")
