...
template <u32 P> static int order(u32 &x) {
  int c = 0;
  while (x % P == 0) {
    ++c;
    x /= P;
...
Primes::Primes(u32 limit) : limit(limit), primeMap(limit / 2) {
  primes.push_back(2);
  primeMap[0] = true;
  u32 last = 0;
  while (true) {
    u32 p = last + 1;
    while (p < limit / 2 && primeMap[p]) {
      ++p;
    }
    u32 prime = 2 * p + 1;
    if (prime >= limit) {
    ...
    last = p;
    u64 s = 2 * u64(p) * (p + 1);
    for (u32 i = s; s < limit / 2 && i < limit / 2; i += prime) {
      primeMap[i] = true;
    }
  }
  primeMap.flip();
...
std::vector<std::pair<u32, u32>> Primes::factors(u32 x) {
  ...
  if (isPrime(x)) {
    ret.push_back(std::make_pair(x, 1u));
    return ret;
  }
...
static void step(u32 p, u32 &d, u32 &r, u32 k) {
  while (r % k == 0 && d != k && modExp2(p, d / k) == 1) {
    d /= k;
    r /= k;
  }
  while (r % k == 0) {
