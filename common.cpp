...
static std::unique_ptr<FILE> open(const std::string &name, const char *mode,
                                  bool doLog) {
...
std::unique_ptr<FILE> openRead(const std::string &name, bool doLog) {
  return open(name, "rb", doLog);
}
std::unique_ptr<FILE> openWrite(const std::string &name) {
  return open(name, "wb", true);
}
std::unique_ptr<FILE> openAppend(const std::string &name) {
  return open(name, "ab", true);
}
...
void log(const char *fmt, ...) {
  ...
  std::string prefix =
      shortTimeStr() + (globalCpuName.empty() ? "" : " ") + globalCpuName;
