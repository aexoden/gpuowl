...
static bool writeResult(const std::string &part, u32 E, const char *workType,
                        const std::string &status, const std::string &AID,
                        const std::string &user, const std::string &cpu) {
  std::string uid;
  if (!user.empty()) {
    uid += ", \"user\":\"" + user + '"';
  }
  ...
  snprintf(buf, sizeof(buf),
           "{\"exponent\":\"%u\", \"worktype\":\"%s\", \"status\":\"%s\", "
           "\"program\":{\"name\":\"%s\", \"version\":\"%s\"}, "
           "\"timestamp\":\"%s\"%s%s%s}",
           E, workType, status.c_str(), PROGRAM, VERSION, timeStr().c_str(),
           uid.c_str(), aidJson.c_str(), part.c_str());
...
static std::string factorStr(const std::string &factor) {
  return factor.empty() ? "" : (", \"factors\":[\"" + factor + "\"]");
}
